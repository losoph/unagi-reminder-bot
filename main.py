...
async def fetch_subscription_posts(
    ...
    next_send_str = get_next_digest_time(period, datetime.now(TZ)).strftime('%Y-%m-%d %H:%M:%S')
...
async def handle_cancel(callback: CallbackQuery):
    ...
    delete_message(callback.message.chat.id, db_id)
...
async def handle_unsub(callback: CallbackQuery):
    ...
    delete_subscription(callback.message.chat.id, sub_id)
...
async def read_saved(callback: CallbackQuery):
    ...
    msg_data = get_saved_message_by_id(callback.message.chat.id, db_id)
    if not msg_data:
...
async def handle_del_saved(callback: CallbackQuery, state: FSMContext):
    db_id = int(callback.data.split("_")[2])
    delete_saved_message(callback.message.chat.id, db_id)
...
async def cmd_test_digest(message: types.Message, state: FSMContext):
    ...
    async with aiohttp.ClientSession(timeout=REQUEST_TIMEOUT) as session:
        async def load_posts(subscription):
            _, username, title, _, _ = subscription
            async with semaphore:
                return title, username, await get_latest_posts(username, last_24h_str, session=session)

        results = await asyncio.gather(*(load_posts(sub) for sub in user_subs), return_exceptions=True)
...
async def check_messages():
    while True:
        ...
        for db_id, chat_id, message_id, retry_count in pending:
            ...
            try:
                await bot.forward_message(chat_id=int(chat_id), from_chat_id=int(chat_id), message_id=message_id)
                mark_as_sent(db_id)
