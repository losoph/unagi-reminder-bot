...
- **Дайджесты публичных каналов**: подписка пересылкой поста или ссылкой на канал (`t.me/канал`, `t.me/s/канал`, ссылка на пост, `telegram.me`, `tg://resolve`, `@канал`), периодичность daily/weekly/monthly с настройкой времени.
  ...
  - `/email` — копия дайджестов на почту (адрес подтверждается кодом; вёрстка как в Telegraph; отписка ссылкой из письма).
...
- `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURITY` (`starttls`/`ssl`/`none`), `SMTP_USER`, `SMTP_PASSWORD`, `SMTP_FROM` - SMTP для email-дайджестов (тот же аккаунт, что у msmtp-алертов); без `SMTP_HOST` функция скрыта. Вместо них можно смонтировать msmtp-конфиг в `MSMTPRC_PATH`
